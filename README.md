...
上述命令适用于无显卡主机：数据集、标注、导出、模型上传、真实 YOLOv5/YOLOv8 检测、YOLOv8-Seg、YOLOv8-Pose、SegFormer/U-Net/DeepLabV3+、HRNet/HigherHRNet CPU 训练，以及 FP32 ONNX、TorchScript、OpenVINO 转换均可用。具备 NVIDIA GPU 时，在 `.env` 设置 `FORGE_GPU_ENABLED=true`，并使用 `docker compose --profile gpu up -d --build` 启动 GPU Worker，以开放 TensorRT、GPU 训练和 SDXL LoRA 训练/转换能力。

浏览器访问 `http://<服务器地址>:5173/`。首次启动只会创建默认工作空间和一个由环境变量指定的管理员账号，业务数据保持为空，详见部署说明。
...
- `/`：工作台、训练运行、资源与数据准备度
- `/datasets`：数据中心入口；管理员预留为数据树页面，标注员和审核员从这里查看数据集与标注进度
- `/tasks`：任务中心；管理员在此管理数据集任务，管理员/审核员可查看任务段详情（左侧导航名称为“标注任务”）
- `/annotate/:datasetId?image=:imageId`：矩形、多边形、线段/折线、椭圆、关键点、骨架与 SDXL Caption 标注工作台；管理员/审核员可使用 `mode=review` 进入审核
...
- 标注支持数据集级统一提交、图像级审核。提交后的图像在审核完成前锁定；管理员/审核员可逐张或批量通过/驳回，驳回必须填写原因。只有全部图像审核通过的数据集才允许训练；导出按整个数据集处理，仅包含已审核且有内容的图片。
...
- 数据集导出产物为 ZIP，按数据集范围生成，且只包含所选格式可表达、已审核并有标注内容的图片；未标注图片会自动跳过。目标检测支持 YOLO、COCO、VOC；语义分割支持 COCO Segmentation、PNG Mask；关键点支持 COCO Keypoints（首版每张图一个实例）；SDXL 支持带 `metadata.jsonl` 的 Image Folder。
- YOLOv5u/YOLOv8 检测、YOLOv8-Seg 和 YOLOv8-Pose 使用 Ultralytics 训练器；YOLOv8-Seg 使用 `YOLO_SEGMENTATION` 多边形 TXT，YOLOv8-Pose 使用 `YOLO_KEYPOINTS` 边框/关键点 TXT。其他分割执行器将矩形/多边形标注栅格化后训练 SegFormer、U-Net 或 DeepLabV3+；其他关键点执行器将编号关键点生成高斯热图后训练 HRNet/HigherHRNet。SegFormer MiT、U-Net/DeepLabV3+ ResNet 编码器和 HRNet 主干均可选择官方预训练权重或从头训练。YOLO 任务登记 Ultralytics `best.pt` 与真实指标，结构化视觉任务登记可加载 TorchScript；所有视觉模型均支持 ONNX、TorchScript、OpenVINO，并在 GPU Worker 上支持 TensorRT。
