...
async function prepareVocDetection(input: TrainingFormatInput, packageDir: string, manifest: FormatManifest) {
  ...
  for (const source of manifest.images) {
    ...
    const annotations = asArray(parsed.annotation?.object).flatMap((item, index) => {
      ...
      if (!box) return [];
      const record = rectangle(item.name ?? input.dataset.classes[0], [Number(box.xmin), Number(box.ymin), Number(box.xmax) - Number(box.xmin), Number(box.ymax) - Number(box.ymin)], source.width, source.height, `voc-${source.id}-${index}`);
      return record ? [record] : [];
    });
