...
export function selectAnnotatedExportData(images: ExportImage[], documents: ExportAnnotationDocument[]) {
  const annotationsByImage = new Map(documents.map((document) => [document.imageId, document.annotations]));
  const selectedImages = images.filter((image) => (annotationsByImage.get(image.id)?.length ?? 0) > 0);
  const selectedImageIds = new Set(selectedImages.map((image) => image.id));
  return {
    images: selectedImages,
    documents: documents.filter((document) => selectedImageIds.has(document.imageId) && document.annotations.length > 0),
...
export function selectFormatCompatibleExportData(format: DataFormat, images: ExportImage[], documents: ExportAnnotationDocument[]) {
  ...
  return selectAnnotatedExportData(images, compatibleDocuments);
}
...
function pointsFor(annotation: AnnotationRecord, width: number, height: number) {
  ...
  if (geometry.type === 'polygon' || geometry.type === 'polyline') return geometry.points.map((point) => ({ x: pixel(point.x, width), y: pixel(point.y, height) }));
...
export async function materializeDatasetFormat(input: DatasetExportInput, packageDir = input.outputDir): Promise<MaterializedDatasetFormat> {
  const selected = selectFormatCompatibleExportData(input.format, input.images, input.documents);
  ...
  await writeFile(manifestPath, JSON.stringify({
    ...
    includeImages: input.includeImages, annotatedOnly: true, imageCount: images.length,
