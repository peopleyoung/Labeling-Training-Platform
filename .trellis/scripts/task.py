...
Usage:
    /home/hdzk/.local/bin/python3.12 task.py create "<title>" --description "<desc>" [--slug <name>] [--assignee <dev>] [--priority P0|P1|P2|P3] [--parent <dir>] [--package <pkg>] [--no-start] [--force]
    /home/hdzk/.local/bin/python3.12 task.py add-context <dir> <file> <path> [reason] # Add jsonl entry
    /home/hdzk/.local/bin/python3.12 task.py validate <dir>              # Validate jsonl files
    /home/hdzk/.local/bin/python3.12 task.py list-context <dir>          # List jsonl entries
    /home/hdzk/.local/bin/python3.12 task.py start <dir>                 # Set active task, record current branch
    /home/hdzk/.local/bin/python3.12 task.py current [--source] [--json] # Show active task
    /home/hdzk/.local/bin/python3.12 task.py finish                      # Clear active task
    /home/hdzk/.local/bin/python3.12 task.py set-branch <dir> <branch>   # Set git branch
    /home/hdzk/.local/bin/python3.12 task.py set-base-branch <dir> <branch>  # Set PR target branch
    /home/hdzk/.local/bin/python3.12 task.py set-scope <dir> <scope>     # Set scope for PR title
    /home/hdzk/.local/bin/python3.12 task.py set-meta <dir> <key> <value>  # Set a task metadata key
    /home/hdzk/.local/bin/python3.12 task.py rename <dir> <new-slug> [--dry-run]  # Rename task + references
    /home/hdzk/.local/bin/python3.12 task.py archive <task-dir> [--skip-branch-validation]  # Archive completed task
    /home/hdzk/.local/bin/python3.12 task.py list                        # List active tasks
    /home/hdzk/.local/bin/python3.12 task.py list-archive [month]        # List archived tasks
    /home/hdzk/.local/bin/python3.12 task.py add-subtask <parent-dir> <child-dir>     # Link child to parent
    /home/hdzk/.local/bin/python3.12 task.py remove-subtask <parent-dir> <child-dir>  # Unlink child from parent
"""
...
def _record_start_state(
    ...
    if base_branch_conflict:
        ...
        print(
            f"Once you branch off, run: /home/hdzk/.local/bin/python3.12 {DIR_WORKFLOW}/scripts/task.py "
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "allow_empty_context", False):
        ...
        if empty_manifests:
            ...
            print(f"  Curate:  /home/hdzk/.local/bin/python3.12 .trellis/scripts/task.py add-context {task_input} implement <path> \"<why>\"")
            print(f"  Verify:  /home/hdzk/.local/bin/python3.12 .trellis/scripts/task.py validate {task_input}")
...
Usage:
  /home/hdzk/.local/bin/python3.12 task.py create <title> --description <desc>  Create new task directory (both required, non-empty)
  /home/hdzk/.local/bin/python3.12 task.py create <title> --description <desc> --package <pkg>   Create task for a specific package
  /home/hdzk/.local/bin/python3.12 task.py create <title> --description <desc> --parent <dir>    Create task as child of parent
  /home/hdzk/.local/bin/python3.12 task.py create <title> --description <desc> --no-start        Create without making it active in this session
  /home/hdzk/.local/bin/python3.12 task.py add-context <dir> <jsonl> <path> [reason]  Add entry to jsonl
  /home/hdzk/.local/bin/python3.12 task.py validate <dir>                     Validate jsonl files
  /home/hdzk/.local/bin/python3.12 task.py list-context <dir>                 List jsonl entries
  /home/hdzk/.local/bin/python3.12 task.py start <dir>                        Set active task; records the checked-out branch when unset
  /home/hdzk/.local/bin/python3.12 task.py current [--source]                 Show active task
  /home/hdzk/.local/bin/python3.12 task.py finish                             Clear active task
  /home/hdzk/.local/bin/python3.12 task.py set-branch <dir> <branch>          Set git branch
  /home/hdzk/.local/bin/python3.12 task.py set-base-branch <dir> <branch>     Set PR target branch
  /home/hdzk/.local/bin/python3.12 task.py set-scope <dir> <scope>            Set scope for PR title
  /home/hdzk/.local/bin/python3.12 task.py set-meta <dir> <key> <value>       Set/overwrite a task metadata key
  /home/hdzk/.local/bin/python3.12 task.py rename <dir> <new-slug>            Rename task, identity fields and references
  /home/hdzk/.local/bin/python3.12 task.py archive <task-dir>                 Archive completed task
  /home/hdzk/.local/bin/python3.12 task.py add-subtask <parent> <child>       Link child task to parent
  /home/hdzk/.local/bin/python3.12 task.py remove-subtask <parent> <child>    Unlink child from parent
  /home/hdzk/.local/bin/python3.12 task.py list [--mine] [--status <status>] [--json]  List tasks
  /home/hdzk/.local/bin/python3.12 task.py list-archive [YYYY-MM]             List archived tasks
...
Examples:
  /home/hdzk/.local/bin/python3.12 task.py create "Add login feature" --description "Email + password sign-in" --slug add-login
  /home/hdzk/.local/bin/python3.12 task.py create "Add login feature" --description "Email + password sign-in" --slug add-login --package cli
  /home/hdzk/.local/bin/python3.12 task.py create "Add login feature" --description "Email + password sign-in" --meta linear=ENG-123 --meta epic=auth
  /home/hdzk/.local/bin/python3.12 task.py create "Child task" --description "Session cookie handling" --slug child --parent .trellis/tasks/01-21-parent
  /home/hdzk/.local/bin/python3.12 task.py add-context <dir> implement .trellis/spec/cli/backend/auth.md "Auth guidelines"
  /home/hdzk/.local/bin/python3.12 task.py set-branch <dir> task/add-login
  /home/hdzk/.local/bin/python3.12 task.py start .trellis/tasks/01-21-add-login
  /home/hdzk/.local/bin/python3.12 task.py current --source
  /home/hdzk/.local/bin/python3.12 task.py finish
  /home/hdzk/.local/bin/python3.12 task.py rename add-login add-sso --dry-run  # Preview the change set
  /home/hdzk/.local/bin/python3.12 task.py rename add-login add-sso
  /home/hdzk/.local/bin/python3.12 task.py archive add-login
  /home/hdzk/.local/bin/python3.12 task.py archive add-login --skip-branch-validation  # Task never had a branch of its own
  /home/hdzk/.local/bin/python3.12 task.py add-subtask parent-task child-task  # Link existing tasks
  /home/hdzk/.local/bin/python3.12 task.py remove-subtask parent-task child-task
  /home/hdzk/.local/bin/python3.12 task.py list                               # List all active tasks
  /home/hdzk/.local/bin/python3.12 task.py list --mine                        # List my tasks only
  /home/hdzk/.local/bin/python3.12 task.py list --mine --status in_progress   # List my in-progress tasks
""")
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "init-context":
        ...
        print(
            "  /home/hdzk/.local/bin/python3.12 ./.trellis/scripts/get_context.py --mode phase --step 1",
            file=sys.stderr,
        )
        print(
