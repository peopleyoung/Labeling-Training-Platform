...
export function TrainingDetailPage() {
  ...
  return (
    <div className="page training-detail-page">
      ...
      {tab === 'overview' && (
        <div className="detail-grid">
          ...
          <section className="panel task-summary-panel">
            ...
            <dl><div><dt><Database size={15} />数据集</dt><dd>{job.dataset}</dd></div><div><dt><Sparkles size={15} />任务类型</dt><dd>{taskLabels[job.type]}</dd></div><div><dt><FileText size={15} />数据格式</dt><dd>{job.config ? dataFormatDescriptions[job.config.dataFormat].label : '--'}</dd></div><div><dt><FileText size={15} />基础模型</dt><dd>{job.model}</dd></div><div><dt><Server size={15} />计算资源</dt><dd>{job.gpu || '--'}</dd></div></dl>
          </section>
        </div>
      )}
      ...
      {tab === 'config' && <section className="config-detail-panel"><div><h2>训练配置</h2><p>任务创建时持久化的完整超参数</p></div>{job.config ? <dl className="config-grid"><div><dt>数据格式</dt><dd>{dataFormatDescriptions[job.config.dataFormat].label}</dd></div><div><dt>权重来源</dt><dd>{job.config.weightSource === 'scratch' ? '从头训练' : '官方预训练权重'}</dd></div><div><dt>训练轮次</dt><dd>{job.config.epochs}</dd></div><div><dt>批次大小</dt><dd>{job.config.batchSize}</dd></div><div><dt>学习率</dt><dd>{job.config.learningRate}</dd></div><div><dt>输入尺寸</dt><dd>{job.config.imageSize}</dd></div><div><dt>混合精度</dt><dd>{job.config.mixedPrecision ? '启用' : '关闭'}</dd></div><div><dt>早停</dt><dd>{job.config.earlyStopping ? '启用' : '关闭'}</dd></div></dl> : <EmptyState icon={FileText} title="暂无完整配置" description="旧任务未保存完整训练参数。" />}</section>}
