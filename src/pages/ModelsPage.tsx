...
import { Archive, ArrowRight, Boxes, Braces, ChevronDown, Download, GitBranch, Plus, Search, ShieldCheck, Sparkles, Upload } from 'lucide-react';
...
export function ModelsPage() {
  ...
  const { models: modelVersions, uploadModel, updateModelStage, downloadArtifact, notify } = useApp();
  const models = useMemo(() => modelVersions.filter((model) => (filter === 'all' || model.task === filter) && `${model.name}${model.version}${model.framework}`.toLowerCase().includes(query.toLowerCase())).sort((left, right) => newestFirst ? right.createdAt.localeCompare(left.createdAt) : left.createdAt.localeCompare(right.createdAt)), [modelVersions, filter, newestFirst, query]);
  ...
  return (
    <div className="page models-page">
      ...
      <section className="data-toolbar">
        <div className="segmented tabs"><button className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>全部模型 <span>{modelVersions.length}</span></button><button className={filter === 'detection' ? 'active' : ''} onClick={() => setFilter('detection')}>目标检测 <span>{detectionCount}</span></button><button className={filter === 'segmentation' ? 'active' : ''} onClick={() => setFilter('segmentation')}>语义分割 <span>{segmentationCount}</span></button><button className={filter === 'keypoint' ? 'active' : ''} onClick={() => setFilter('keypoint')}>关键点 <span>{keypointCount}</span></button></div>
        <div className="toolbar-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索模型与版本" /></div>
        <button className="button secondary compact" onClick={() => setNewestFirst((value) => !value)}>{newestFirst ? '最近更新' : '最早创建'} <ChevronDown size={14} /></button>
      </section>
        ...
        <div className="table-scroll">
          <table className="data-table models-table">
            ...
            <tbody>
              {models.map((model) => (
                <tr key={model.id}>
                  <td><div className="model-name-cell"><span className={`model-glyph ${model.task}`}><Sparkles size={20} /></span><div><strong>{model.name}</strong><span><GitBranch size={13} />来自 {model.sourceJob}</span></div></div></td>
                  <td><strong className="model-version">{model.version}</strong><span className="cell-subtext">{model.size}</span></td>
                  ...
                  <td><div className="row-actions">{model.artifactId && <button className="icon-button bordered" title="下载模型" aria-label={`下载 ${model.name}`} onClick={() => void download(model)}><Download size={16} /></button>}<Link className="button compact secondary" to={`/conversions?source=${model.id}`}>转换 <ArrowRight size={15} /></Link><button className="icon-button" title={model.stage === '已归档' ? '恢复评估' : '归档模型'} aria-label={model.stage === '已归档' ? `恢复 ${model.name}` : `归档 ${model.name}`} onClick={() => void toggleArchive(model)}><Archive size={17} /></button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        {models.length === 0 && <EmptyState icon={Boxes} title={modelVersions.length ? '没有匹配的模型' : '暂无模型版本'} description={modelVersions.length ? '调整搜索条件后重试' : '完成训练或上传模型后，版本会出现在这里'} />}
      </section>
      ...
      {lineageOpen && <Modal title="模型资产谱系" description="训练任务或人工上传来源，以及当前部署格式。" onClose={() => setLineageOpen(false)} footer={<button className="button primary" onClick={() => setLineageOpen(false)}>关闭</button>}><div className="lineage-list">{modelVersions.map((model) => <div key={model.id}><span className={`model-glyph ${model.task}`}><GitBranch size={17} /></span><span><strong>{model.name} {model.version}</strong><small>来源：{model.sourceJob} · {model.framework}</small></span><span>{model.formats.length ? model.formats.join(' / ') : '原始模型'}</span></div>)}</div></Modal>}
    </div>
  );
}
