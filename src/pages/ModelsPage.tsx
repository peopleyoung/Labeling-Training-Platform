...
export function ModelsPage() {
  ...
  const instanceSegmentationCount = modelVersions.filter((model) => model.task === 'instance_segmentation').length;
            ...
            <tbody>
              {models.map((model) => (
                <tr key={model.id}>
                  ...
                  <td><span className="neutral-badge">{taskLabels[model.task]}</span><span className="cell-subtext">{model.framework}</span><span className="cell-subtext">{model.architectureVariant === 'rk_compatible' ? 'RK 友好结构' : '通用结构'} · {model.rkCompatibilityStatus === 'rk_structure_ready' ? 'RK 结构已就绪' : model.rkCompatibilityStatus === 'onnx_validated' ? 'ONNX 已验证' : model.rkCompatibilityStatus === 'device_validated' ? '设备已验证' : '未承诺 RK 兼容'}</span></td>
